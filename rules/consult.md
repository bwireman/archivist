...
2. Generated tree when MCP is unavailable (written only if `records.write_docs` is true): `docs/archive/INDEX.md`, then the type digest files (`docs/archive/rules.md`, `features.md`, `decisions.md`, …) and copies under `docs/archive/records/<scope>/<type>/`, then the configured `records.repo` directory and `records.global` (default `~/.archivist`)
3. Code last
