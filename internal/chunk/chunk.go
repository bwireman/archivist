...
func Classify(path string, repoADR, globalADR []string) (Type, string) {
	...
	if glob.MatchAnyPattern(path, globalADR) {
		return TypeADR, ScopeGlobal
	}
	if glob.MatchAnyPattern(path, repoADR) {
...
// MatchAnyPattern delegates to internal/glob.
func MatchAnyPattern(path string, patterns []string) bool {
	return glob.MatchAnyPattern(path, patterns)
}
